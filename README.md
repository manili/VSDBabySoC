...
In mixed-signal design, we instantiate the analog IP cores (e.g. PLL and DAC) as a black-box, and the synthesizer and STA tool will use .lib info to do their job. Unfortunately, there is no free open-source tool to extract the timing and power info from the final spice model. So we'll use a fake .lib file for out IP cores that will be generated by a perl script.
